...
{	
	...
	{
		printf("JE ME SUIS CARSHE MAYDAY");
		pointeur->crash = 1;
	}
	if( pointeur->crash == 1)
	{ 
...
{	
	...
	{	
		printf("MAYDAYAMDAYDAYD JE ME SUIS CRASHE 22222222");
		 pointeur->crash = 1;
	}
	if( pointeur->crash == 1 )
	{
...
{		
	...
	{
		printf("JEPOKEO MAYDAY 3333333333333333333");
		pointeur->crash = 1;
	}
	if( pointeur->crash == 1)
	{
...
int i;
int proba;
int voitures[20] = {44, 77, 3, 33, 5, 7, 11, 31, 19, 18, 14, 2, 10, 55, 8, 20, 27, 30, 9, 94};	
int meilleurVoitS1;
...
// 4) CREATION D'UNE "voiture"
voiture a; // mémoire allouée en même temps que création de la struct voiture a
...
for( proba = a.tourRestant / 2, i = 1 ; i < 50 ; i++, proba--)
{	
	...
	a.s3 = (double) secteur3( i, &a )/1000.0;
		...
		if(a.crash != 1){
			if(a.pit==0 && pit(proba) == 1 ){  // SI a.pit = 0    ET    pit(.....) est égal à 1 ALORS ==> ajoute temp pit
				...
				srand(time(NULL));
				a.s3 += (double)(rand()%6000 + 25000)/1000.0;
